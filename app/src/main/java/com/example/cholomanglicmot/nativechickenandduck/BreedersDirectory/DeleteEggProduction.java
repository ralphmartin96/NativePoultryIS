...
public class DeleteEggProduction extends DialogFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.dialog_delete_egg_production, container, false);


        final Integer feeding_id = getArguments().getInt("Breeder Egg Prod ID");
        context = getActivity();
        ...
        yes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                boolean isDeleted = myDb.deleteEggProduction(feeding_id);
                if(isNetworkAvailable()){

                    RequestParams requestParams = new RequestParams();
                    requestParams.add("ID", feeding_id.toString());
                    requestParams.add("date", getDate());



                    API_deleteEggProduction(requestParams);
                }
                if(isDeleted){
                    Toast.makeText(getActivity(), "Succesfully deleted egg production record", Toast.LENGTH_SHORT).show();
                    getDialog().dismiss();
    ...
    private void API_deleteEggProduction(RequestParams requestParams){
        APIHelperAsync.deleteEggProduction("deleteEggProduction", requestParams, new BaseJsonHttpResponseHandler<Object>() {
            @Override
