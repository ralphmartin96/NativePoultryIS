...
*/
    ...
    public Cursor getAllDataFromReplacementGrowthWhereInvID(Integer id) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from " + TABLE_REPLACEMENT_GROWTH_RECORDS + " where REPLACEMENT_GROWTH_INVENTORY_ID is ?", new String[]{id.toString()});
...
//        Cursor res = db.rawQuery("SELECT ID FROM " +TABLE_BROODER_INVENTORIES+ " WHERE BROODER_INV_PEN_NUMBER is ? AND BROODER_INV_DELETED_AT IS NULL", new String[]{String.valueOf(pen_id)});
    ...
    public int getBroodersGrowthSize(){
        SQLiteDatabase db = this.getWritableDatabase();
        int size=0;

        Cursor res = db.rawQuery("SELECT COUNT(*) FROM "+TABLE_BROODER_GROWTH_RECORDS, new String[]{});
        res.moveToFirst();
    ...
    public int getBreederFeedingSize(){
        SQLiteDatabase db = this.getWritableDatabase();
        int size=0;

        Cursor res = db.rawQuery("SELECT COUNT(*) FROM "+TABLE_BREEDER_FEEDING_RECORDS, new String[]{});
        res.moveToFirst();
    ...
    public int getHatcherySize(){
        SQLiteDatabase db = this.getWritableDatabase();
        int size=0;

        Cursor res = db.rawQuery("SELECT COUNT(*) FROM "+TABLE_HATCHERY_RECORD, new String[]{});
        res.moveToFirst();
